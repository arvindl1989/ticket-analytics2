...
import { getBandwidth, getBandwidthRates, updateBandwidthRates } from '../api'
...
export default function BandwidthPage({ sessionId, onSessionExpired }) {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      avgLoad:       members.length ? Math.round(members.reduce((s, m) => s + m.load_pct, 0) / members.length) : 0,
  ...
  // Chart data — horizontal bars per person (committed vs available)
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr
                  key={m.assigned_to}
                  style={{ background: i % 2 === 0 ? '#fff' : '#fafafa', borderBottom: '1px solid #f0f3fa', transition: 'background 0.1s' }}
